...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      const topMenuEl = this.querySelector(
        "set-radius-form-comp"
      ) as HTMLElement;
      const formRadiusEl = topMenuEl.querySelector(
        ".search-form"
      ) as HTMLElement;
      ...
      formRadiusEl.addEventListener("submit", async (e: any) => {
        e.preventDefault();
        ...
        fetch(
          ...
          .then((results) => {
            for (const p of results) {
              if (p.found == false && p.userId != cs.userId) {
                const { lat, lng } = p._geoloc;
                const marker = new mapboxgl.Marker({ color: "#eb6372" })
                  .setLngLat([lng, lat])
                  .addTo(map);
                state.getPetsByRadius(results);

                marker.getElement().addEventListener("click", async () => {
                  const { lng, lat } = marker._lngLat;

                  const selectedPetOnMap = cs.petsByRadius.find((p) => {
    ...
    render() {
      this.innerHTML = `
            <set-radius-form-comp></set-radius-form-comp>
    ...
    connectedCallbackSelectedPet() {
      this.renderSelectedPet();
      const cs = state.getState();

      const mapEl = this.querySelector("#map") as HTMLElement;
      const missingPetCardEl = this.querySelector(
        "missing-pet-card-comp"
      ) as HTMLElement;
      const exitButtonEl = missingPetCardEl.querySelector(
        ".exit-button"
      ) as HTMLElement;
      const cardContainerEl = missingPetCardEl.querySelector(
        ".card__container"
      ) as HTMLElement;
      const reportButtonEl = missingPetCardEl.querySelector(
        ".card__button"
      ) as HTMLElement;
    ...
    renderSelectedPet() {
      this.innerHTML = `
            <div class="map-container">
                <missing-pet-card-comp id="${this.petId}" picURL="${this.picURL}" name="${this.name}" details="${this.details}"></missing-pet-card-comp>
                <div id="map"></div>
            </div>
          `;
